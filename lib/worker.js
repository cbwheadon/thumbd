...
setupSQSMessage = function() {
	...
	config.set('sqsReplyUrl', sqs.endpoint.protocol + '//' + sqs.endpoint.hostname + '/' + config.get('sqsReply'));
	waitforMessage();
};
...
Worker.prototype._sendReply = function(id, queue, files, qrcode){
	...
	config.set('sqsReplyUrl', sqs.endpoint.protocol + '//' + sqs.endpoint.hostname + '/' + queue + '_' + config.get('sqsReply'));
	sqs.sendMessage({QueueUrl: config.get('sqsReplyUrl'), MessageBody: JSON.stringify({
		id: id,
		files: files,
		qrcode: qrcode
	})}, function (err, result) {
...
Worker.prototype._deleteJob = function(handle) {
	sqs.deleteMessage({QueueUrl: config.get('sqsQueueUrl'), ReceiptHandle: handle}, function(err, resp) {
		if (err) {
